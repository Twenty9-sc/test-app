...
# if os.name == "nt":
#     DOSSIER_DONNEES = os.environ.get("BV_DATA_DIR", r"C:\BV\data")
#     DOSSIER_BACKUPS = os.environ.get("BV_BACKUP_DIR", r"C:\BV\backups")
# else:
#     DOSSIER_DONNEES = os.environ.get("BV_DATA_DIR", "/data")
#     DOSSIER_BACKUPS = os.environ.get("BV_BACKUP_DIR", "/backups")

# FICHIER_SAUVEGARDE = os.path.join(DOSSIER_DONNEES, "donnees_BV.json")

#####################################################################      revoir charger_donnees & sauvegarder_donnees
...
if "current_user" not in st.session_state:
    st.session_state.current_user = "Utilisateur B&V"
...
# =============================================================================
# CHEMINS DES DONNÉES — SQLite compatible Windows / Streamlit Cloud / Synology
# =============================================================================

def lire_config(cle, defaut=""):
    """
    Lit une configuration depuis :
    1. Streamlit secrets
...
def dossier_est_utilisable(dossier):
    """
    Vérifie qu'un dossier peut être créé et qu'on peut écrire dedans.
    """
    try:
        os.makedirs(dossier, exist_ok=True)

        fichier_test = os.path.join(dossier, ".test_ecriture_bv")

        with open(fichier_test, "w", encoding="utf-8") as fichier:
            fichier.write("ok")
...
def choisir_dossier_utilisable(dossier_demande, nom_fallback):
    """
    Choisit un dossier d'écriture valide.
    Évite les erreurs PermissionError sur Streamlit Cloud.
    """
    candidats = []

    if dossier_demande:
        candidats.append(dossier_demande)

    candidats.append(os.path.join(DOSSIER_APP, nom_fallback))

    if os.name != "nt":
        candidats.append(os.path.join(tempfile.gettempdir(), f"bv_{nom_fallback}"))

    for dossier in candidats:
        if dossier_est_utilisable(dossier):
            return dossier
...
if os.name == "nt":
    ...
    DOSSIER_BACKUPS_DEMANDE = lire_config("BV_BACKUP_DIR", r"C:\BV\backups")
else:
    # Linux : Streamlit Cloud / Synology / Docker
    # Ne PAS utiliser /data par défaut sur Streamlit Cloud.
    DOSSIER_DONNEES_DEMANDE = lire_config(
        "BV_DATA_DIR",
        os.path.join(DOSSIER_APP, "data"),
    )
    DOSSIER_BACKUPS_DEMANDE = lire_config(
        "BV_BACKUP_DIR",
        os.path.join(DOSSIER_APP, "backups"),
    )


DOSSIER_DONNEES = choisir_dossier_utilisable(
    DOSSIER_DONNEES_DEMANDE,
    "data",
)

DOSSIER_BACKUPS = choisir_dossier_utilisable(
    DOSSIER_BACKUPS_DEMANDE,
    "backups",
)
...
FICHIER_SQLITE = os.path.join(DOSSIER_DONNEES, "donnees_BV.sqlite")

# Alias pour éviter toutes les erreurs "undefined variable"
FICHIER_SQLITE_LEGACY = FICHIER_SQLITE

# Le fichier principal de l'application est maintenant SQLite
FICHIER_SAUVEGARDE = FICHIER_SQLITE

# Si ton ancien système de session utilise cette variable, elle est définie aussi
FICHIER_SESSIONS = os.path.join(DOSSIER_DONNEES, "sessions_BV.json")

# On force le backend local SQLite.
# Ça évite que le code essaie d'appeler PostgreSQL sans configuration.
...
# Connexion désactivée : accès direct au portail
st.session_state.logged_in = True
st.session_state.setdefault("current_user", "Utilisateur BV")
...
if REPORTLAB_DISPO:
    ...
    class NumeroteurDePages(canvas.Canvas):
        ...
        def __init__(
            self,
            *args,
            signature_redacteur=None,
            signature_direction="Responsable Qualité",
            afficher_signature_derniere_page=False,
            **kwargs,
        ):
            super().__init__(*args, **kwargs)
            self.pages = []
            self.signature_redacteur = signature_redacteur
            self.signature_direction = signature_direction
            self.afficher_signature_derniere_page = afficher_signature_derniere_page

        def showPage(self):
            self.pages.append(dict(self.__dict__))
            self._startPage()

        def _dessiner_signature_derniere_page(self):
            """
            Dessine le bloc Visa / Approbation tout en bas de la dernière page.
            """
            if not self.afficher_signature_derniere_page:
                return

            try:
                largeur_page, hauteur_page = self._pagesize

                marge_gauche = 50
                largeur_disponible = largeur_page - 100

                y_signature = 55

                redacteur_txt = html_lib.escape(
                    html_lib.unescape(str(self.signature_redacteur or "Labo / R&D"))
                )

                direction_txt = html_lib.escape(
                    html_lib.unescape(str(self.signature_direction or "Responsable Qualité"))
                )

                style_signature = ParagraphStyle(
                    "StyleSignatureBasPage",
                ...
                data_sign = [
                    [
                        Paragraph(
                            f"<b>Visa Rédacteur / Émetteur :</b><br/>{redacteur_txt}",
                            style_signature,
                        ),
                        Paragraph(
                            f"<b>Approbation Direction :</b><br/>{direction_txt}",
                            style_signature,
                        ),
                    ]
                ]
                ...
                t_sign = Table(
                    data_sign,
                    colWidths=[largeur_disponible / 2, largeur_disponible / 2],
                )
                ...
                largeur_table, hauteur_table = t_sign.wrapOn(
                    self,
                    largeur_disponible,
                    60,
                )
        ...
        def save(self):
            ...
            for index_page, page in enumerate(self.pages, start=1):
                ...
                if index_page == nombre_pages:
                    self._dessiner_signature_derniere_page()
...
# =============================================================================
# NORMALISATION DES TEXTES — PREMIÈRE LETTRE EN MAJUSCULE
# =============================================================================

CHAMPS_MAJUSCULE_INITIALE = {
    # Noms / titres / désignations
    "nom",
    "nom_actuel",
    "nom_futur",
    "titre",
    ...
    # Sociétés / fournisseurs / groupes
    "societe",
    "fournisseur",
    "sous_groupe",

    # Mélanges / emplacements / catégories
    "categorie_choisie",
    "emplacement",

    # Commentaires / descriptions / notes
    "commentaire",
    "commentaire_global",
    ...
    # Marquages
    "couleur_texte_libelle",
    "marquage_libelle",
    "marquage_description",    
    
}

CHAMPS_LISTES_MAJUSCULE_INITIALE = {
    "compartiments",
    "sous_groupes",
    "ateliers",
}
...
def majuscule_initiale(valeur):
    """
    Met uniquement la première lettre alphabétique en majuscule.
    Le reste du texte reste exactement comme saisi.

    Exemples :
    - "mauve Vj a" -> "Mauve Vj a"
    - "a" -> "A"
    - "  résine rapide" -> "Résine rapide"
    - "RAL 5015" -> "RAL 5015"
    """
    if valeur is None:
        return valeur
    ...
    for position, caractere in enumerate(texte):
        if caractere.isalpha():
            return texte[:position] + caractere.upper() + texte[position + 1:]
...
def normaliser_textes_db(objet, cle_parent=""):
    """
    Parcourt les données BOS2 et normalise les champs texte visibles.
    Ne touche pas aux codes, références, RAL, dosages, couleurs, images, ids, etc.
    ...
    if isinstance(objet, dict):
        ...
        # Cas spécial : champs dynamiques de FT.
        # On normalise les intitulés et les valeurs.
        if cle_parent == "specs_dynamiques":
            nouveau_dict = {}

            for cle, valeur in objet.items():
                nouvelle_cle = majuscule_initiale(cle) if isinstance(cle, str) else cle
                ...
                nouveau_dict[nouvelle_cle] = nouvelle_valeur

                if nouvelle_cle != cle or nouvelle_valeur != valeur:
                    modifie = True

            if modifie:
                objet.clear()
                objet.update(nouveau_dict)
        ...
        for cle, valeur in list(objet.items()):
            ...
            if cle in CHAMPS_MAJUSCULE_INITIALE and isinstance(valeur, str):
                ...
                if nouvelle_valeur != valeur:
                    objet[cle] = nouvelle_valeur
                    modifie = True

            # Listes de textes : compartiments, ateliers, sous-groupes
            elif cle in CHAMPS_LISTES_MAJUSCULE_INITIALE and isinstance(valeur, list):
                nouvelle_liste = []

                for element in valeur:
                    if isinstance(element, str):
                        nouvelle_liste.append(majuscule_initiale(element))
                    else:
                        nouvelle_liste.append(element)

                if nouvelle_liste != valeur:
                    objet[cle] = nouvelle_liste
                    modifie = True

            # Parcours récursif
            elif isinstance(valeur, (dict, list)):
                if normaliser_textes_db(valeur, cle):
                    modifie = True

    elif isinstance(objet, list):
        for element in objet:
            if isinstance(element, (dict, list)):
                if normaliser_textes_db(element, cle_parent):
                    modifie = True

    return modifie
# =============================================================================
# BASE DE DONNÉES SQLITE — STOCKAGE CENTRAL DES DONNÉES BV
# =============================================================================

def ouvrir_connexion_db():
    """
    Ouvre la base SQLite.
    WAL améliore la fiabilité quand plusieurs utilisateurs consultent le site.
    """
    os.makedirs(DOSSIER_DONNEES, exist_ok=True)

    conn = sqlite3.connect(FICHIER_SQLITE, timeout=30)
...
def initialiser_db():
    ...
    app_state contient l'état complet actuel de BV.
    app_backups contient des sauvegardes automatiques.
    """
...
def sauvegarder_donnees():
    ...
    try:
        ...
        if ligne_actuelle:
            ancienne_version = int(ligne_actuelle[0] or 1)
            anciennes_donnees = ligne_actuelle[1]
        else:
            ancienne_version = 0
            anciennes_donnees = ""
        ...
        if anciennes_donnees:
            conn.execute(
                """
                INSERT OR IGNORE INTO app_backups (label, version, created_at, data)
                VALUES (?, ?, ?, ?)
                """,
                (
                    label_backup,
...
# =============================================================================
# IMAGES DANS LES COMMENTAIRES GÉNÉRAUX DES FT
# =============================================================================

def encoder_image_commentaire_ft(uploaded_file, taille_max=900):
    """
    Encode une image pour les commentaires généraux de FT.
    Redimensionne l'image pour éviter de trop alourdir le JSON / SQLite.
    """
    if uploaded_file is None:
        return None
    ...
    try:
        ...
        # Conserve la transparence si elle existe, sinon RGB.
        ...
        nom_original = getattr(uploaded_file, "name", "image_commentaire")
        nom_sans_ext = os.path.splitext(nom_original)[0]

        return {
            "name": f"{nom_sans_ext}.{extension}",
            "mime": mime,
            "data": base64.b64encode(buffer.getvalue()).decode("utf-8"),
        }

    except Exception as erreur:
        st.error(f"Erreur lors de l'encodage de l'image FT : {erreur}")
        return None


def gerer_images_commentaire_ft(prefixe, objet_existant=None):
    """
    Interface Streamlit pour gérer les images des commentaires généraux FT.
    - affiche les images existantes ;
    - permet de les supprimer ;
    - permet d'ajouter plusieurs nouvelles images.
    """
    objet_existant = objet_existant or {}

    images_existantes = copy.deepcopy(
        objet_existant.get("commentaire_global_images", [])
        or objet_existant.get("images_commentaire_global", [])
        or objet_existant.get("fiche_technique", {}).get("commentaire_global_images", [])
        or []
    )

    st.markdown("##### Images liées aux commentaires généraux")

    supprimer_images_existantes = False

    if images_existantes:
        with st.expander(
            f"Images actuellement associées ({len(images_existantes)})",
            expanded=False,
        ):
            colonnes = st.columns(3)

            for idx_img, image_info in enumerate(images_existantes):
                with colonnes[idx_img % 3]:
                    afficher_image_base64(image_info.get("data"), width=130)
                    st.caption(image_info.get("name", f"Image {idx_img + 1}"))

        supprimer_images_existantes = st.checkbox(
            "Supprimer les images actuelles des commentaires généraux",
            key=f"{prefixe}_remove_comment_images",
        )

    fichiers_images = st.file_uploader(
        "Ajouter une ou plusieurs images aux commentaires généraux",
        type=["png", "jpg", "jpeg", "webp"],
        accept_multiple_files=True,
        key=f"{prefixe}_comment_images_upload",
    )

    nouvelles_images = []

    for fichier_image in fichiers_images or []:
        image_encodee = encoder_image_commentaire_ft(fichier_image)

        if image_encodee:
            nouvelles_images.append(image_encodee)

    if supprimer_images_existantes:
        return nouvelles_images

    return images_existantes + nouvelles_images
...
def pro_date_maintenant():
    try:
        import zoneinfo
        return datetime.datetime.now(zoneinfo.ZoneInfo("Europe/Paris")).strftime("%d/%m/%Y - %H:%M")
    except Exception:
        try:
            return datetime.datetime.now().astimezone().strftime("%d/%m/%Y - %H:%M")
        except Exception:
...
def normaliser_nom_compartiment(valeur):
    """
    Normalise un nom pour comparer sans tenir compte des majuscules/minuscules.
    """
    return str(valeur or "").strip().casefold()


def nettoyer_nom_compartiment(valeur):
    """
    Nettoie et applique la majuscule initiale au nouveau nom.
    """
    return majuscule_initiale(str(valeur or "").strip())


def nom_existe_deja_dans_liste(liste_valeurs, nouveau_nom, ancien_nom=None):
    """
    Vérifie si un nom existe déjà dans une liste, sauf s'il s'agit du même ancien nom.
    """
    nouveau_norm = normaliser_nom_compartiment(nouveau_nom)
    ancien_norm = normaliser_nom_compartiment(ancien_nom)

    for valeur in liste_valeurs:
        valeur_norm = normaliser_nom_compartiment(valeur)

        if valeur_norm == nouveau_norm and valeur_norm != ancien_norm:
            return True

    return False


def renommer_valeur_dans_liste_principale(liste_valeurs, ancien_nom, nouveau_nom):
    """
    Renomme une valeur dans une liste principale.
    Exemple : preparation["compartiments"] ou preparation["ateliers"].
    """
    ancien_norm = normaliser_nom_compartiment(ancien_nom)

    for index_valeur, valeur in enumerate(list(liste_valeurs)):
        if normaliser_nom_compartiment(valeur) == ancien_norm:
            liste_valeurs[index_valeur] = nouveau_nom

    # Déduplication propre au cas où
    valeurs_finales = []
    deja_vus = set()
    ...
    for valeur in liste_valeurs:
        ...
        if valeur_norm and valeur_norm not in deja_vus:
            valeurs_finales.append(valeur)
            deja_vus.add(valeur_norm)

    liste_valeurs[:] = valeurs_finales


def renommer_valeur_dans_liste_usage(liste_usage, ancien_nom, nouveau_nom):
    """
    Renomme une valeur dans une liste d'utilisation.
    Exemple : element["compartiments"] ou melange["ateliers"].
    """
    ancien_norm = normaliser_nom_compartiment(ancien_nom)
    ...
    for valeur in liste_usage or []:
        if normaliser_nom_compartiment(valeur) == ancien_norm:
            valeur_finale = nouveau_nom
        else:
            valeur_finale = valeur

        valeur_norm = normaliser_nom_compartiment(valeur_finale)

        if valeur_norm and valeur_norm not in deja_vus:
            resultat.append(valeur_finale)
            deja_vus.add(valeur_norm)

    return resultat


def renommer_compartiment_composants(liste_compartiments, liste_additifs, ancien_nom, nouveau_nom):
    """
    Renomme un compartiment du catalogue des composants partout :
    - dans la liste des compartiments ;
    - dans chaque élément qui l'utilise.
    """
    renommer_valeur_dans_liste_principale(
        liste_compartiments,
        ancien_nom,
        nouveau_nom,
    )
    ...
    for element_item in liste_additifs:
        element_item["compartiments"] = renommer_valeur_dans_liste_usage(
            element_item.get("compartiments", []),
            ancien_nom,
            nouveau_nom,
        )


def renommer_compartiment_melanges(liste_ateliers, liste_melanges, ancien_nom, nouveau_nom):
    """
    Renomme un compartiment / atelier des autres mélanges partout :
    - dans la liste des ateliers ;
    - dans chaque mélange qui l'utilise.
    """
    renommer_valeur_dans_liste_principale(
        liste_ateliers,
        ancien_nom,
    ...
    for melange_item in liste_melanges:
        melange_item["ateliers"] = renommer_valeur_dans_liste_usage(
            melange_item.get("ateliers", []),
            ancien_nom,
            nouveau_nom,
        )

# =============================================================================
# MARQUAGE VISUEL DES AUTRES MÉLANGES
# =============================================================================

OPTIONS_COULEURS_AUTRES_MELANGES = [
    {"id": "aucune", "label": "Aucune couleur spéciale", "hex": "#142235"},

    {"id": "rouge_alerte", "label": "Rouge alerte", "hex": "#CC0605"},
    {"id": "rouge_fonce", "label": "Rouge foncé", "hex": "#B91C1C"},
    {"id": "rose_fonce", "label": "Rose foncé", "hex": "#DB2777"},
    {"id": "magenta", "label": "Magenta", "hex": "#BE185D"},
    {"id": "fuchsia", "label": "Fuchsia", "hex": "#C026D3"},

    {"id": "violet", "label": "Violet", "hex": "#7C3AED"},
    {"id": "prune", "label": "Prune", "hex": "#86198F"},
    {"id": "indigo", "label": "Indigo", "hex": "#4338CA"},
    {"id": "bleu_royal", "label": "Bleu royal", "hex": "#1D4ED8"},
    {"id": "bleu_vif", "label": "Bleu vif", "hex": "#0284C7"},

    {"id": "cyan_fonce", "label": "Cyan foncé", "hex": "#0E7490"},
    {"id": "turquoise", "label": "Turquoise", "hex": "#0F766E"},
    {"id": "emeraude", "label": "Émeraude", "hex": "#047857"},
    {"id": "vert_controle", "label": "Vert contrôle", "hex": "#15803D"},
    {"id": "vert_lime_fonce", "label": "Vert lime foncé", "hex": "#4D7C0F"},

    {"id": "vert_acide", "label": "Vert acide", "hex": "#65A30D"},
    {"id": "ambre", "label": "Ambre", "hex": "#D97706"},
    {"id": "orange", "label": "Orange", "hex": "#EA580C"},
    {"id": "corail", "label": "Corail", "hex": "#E11D48"},
    {"id": "framboise", "label": "Framboise", "hex": "#BE123C"},
]


OPTIONS_MARQUAGES_AUTRES_MELANGES = [
    {
        "code": "aucun",
        "emoji": "",
        "libelle": "Aucun marquage",
        "description": "Aucune indication particulière.",
    },
    {
        "code": "nouveau",
        "emoji": "🆕",
        "libelle": "Nouveau",
        "description": "Mélange récemment ajouté ou nouvellement utilisé.",
    },
    {
        "code": "ancien",
        "emoji": "🕰️",
        "libelle": "Ancien",
        "description": "Ancienne référence conservée pour historique ou usage ponctuel.",
    },
    {
        "code": "tres_utilise",
        "emoji": "🔥",
        "libelle": "Très utilisé",
        "description": "Mélange fréquemment utilisé en atelier.",
    },
    {
        "code": "moins_utilise",
        "emoji": "📉",
        "libelle": "Moins utilisé",
        "description": "Mélange encore présent mais peu utilisé.",
    },
    {
        "code": "non_utilise",
        "emoji": "❌",
        "libelle": "Plus utilisé / à retirer",
        "description": "Mélange qui n'est plus utilisé ou à sortir progressivement.",
    },
    {
        "code": "essai_developpement",
        "emoji": "🧪",
        "libelle": "Essai / développement",
        "description": "Mélange en test, développement ou validation technique.",
    },
    {
        "code": "recurrent",
        "emoji": "🔁",
        "libelle": "Récurrent",
        "description": "Mélange qui revient régulièrement dans les besoins.",
    },
    {
        "code": "probleme",
        "emoji": "🐞",
        "libelle": "Problème",
        "description": "Mélange avec anomalie, défaut ou point bloquant constaté.",
    },
    {
        "code": "Nocif",
        "emoji": "☠",
        "libelle": "Nocif",
        "description": "Produit nocif, toxic, polluant ou corrosif ...",
    },
    {
        "code": "attention",
        "emoji": "⚠️",
        "libelle": "Attention",
        "description": "Mélange nécessitant une vigilance particulière.",
    },
    {
        "code": "prioritaire",
        "emoji": "⭐",
        "libelle": "Prioritaire",
        "description": "Mélange important ou prioritaire.",
    },
    {
        "code": "valide",
        "emoji": "✅",
        "libelle": "Validé",
        "description": "Mélange contrôlé et validé.",
    },
    {
        "code": "a_controler",
        "emoji": "👁️",
        "libelle": "À contrôler",
        "description": "Mélange à vérifier avant usage ou fabrication.",
    },
    {
        "code": "a_surveiller",
        "emoji": "📌",
        "libelle": "À surveiller",
        "description": "Mélange à suivre dans le temps.",
    },
    {
        "code": "maintenance",
        "emoji": "🛠️",
        "libelle": "Maintenance / correction",
        "description": "Mélange nécessitant une correction ou une mise à jour.",
    },
    {
        "code": "bloque",
        "emoji": "🔒",
        "libelle": "Bloqué / réservé",
        "description": "Mélange bloqué, réservé ou à ne pas modifier sans accord.",
    },
    {
        "code": "attente",
        "emoji": "⏳",
        "libelle": "En attente",
        "description": "Mélange en attente d'information, validation ou décision.",
    },
    {
        "code": "lancement",
        "emoji": "🚀",
        "libelle": "Lancement",
        "description": "Mélange prévu pour démarrage ou mise en production.",
    },
    {
        "code": "stock",
        "emoji": "📦",
        "libelle": "Stock / approvisionnement",
        "description": "Mélange lié au stock, à l'approvisionnement ou à une disponibilité.",
    },
    {
        "code": "securite",
        "emoji": "🧯",
        "libelle": "Sécurité",
        "description": "Mélange avec point sécurité ou précaution particulière.",
    },
    {
        "code": "amelioration",
        "emoji": "💡",
        "libelle": "Amélioration / idée",
        "description": "Mélange à améliorer ou piste d'optimisation.",
    },
]


def get_option_couleur_autre_melange(valeur=None):
    """
    Récupère une couleur par id ou par code hex.
    """
    valeur = str(valeur or "").strip()

    for option in OPTIONS_COULEURS_AUTRES_MELANGES:
        if valeur == option["id"]:
            return option
        if valeur.lower() == option["hex"].lower():
            return option

    return OPTIONS_COULEURS_AUTRES_MELANGES[0]


def get_option_marquage_autre_melange(code=None, emoji=None, libelle=None):
    """
    Récupère un marquage par code, emoji ou libellé.
    """
    code = str(code or "").strip()
    emoji = str(emoji or "").strip()
    libelle = str(libelle or "").strip().lower()

    for option in OPTIONS_MARQUAGES_AUTRES_MELANGES:
        if code and code == option["code"]:
            return option
        if emoji and emoji == option["emoji"]:
            return option
        if libelle and libelle == option["libelle"].lower():
            return option

    return OPTIONS_MARQUAGES_AUTRES_MELANGES[0]


def get_couleur_depuis_melange(melange_data):
    return get_option_couleur_autre_melange(
        melange_data.get("couleur_marquage")
        or melange_data.get("couleur_texte")
        or melange_data.get("couleur_texte_id")
    )


def get_marquage_depuis_melange(melange_data):
    return get_option_marquage_autre_melange(
        code=melange_data.get("marquage_code"),
        emoji=melange_data.get("marquage_emoji"),
        libelle=melange_data.get("marquage_libelle"),
    )


def valeurs_apparence_autre_melange(couleur_id="aucune", marquage_code="aucun"):
    couleur = get_option_couleur_autre_melange(couleur_id)
    marquage = get_option_marquage_autre_melange(code=marquage_code)

    return {
        "couleur_marquage": couleur["id"],
        "couleur_texte": couleur["hex"],
        "couleur_texte_libelle": couleur["label"],

        "marquage_code": marquage["code"],
        "marquage_emoji": marquage["emoji"],
        "marquage_libelle": marquage["libelle"],
        "marquage_description": marquage["description"],
    }


def afficher_legende_marquages(prefixe):
    """
    Affiche / masque la légende des marquages.
    Version sans HTML pour éviter l'affichage des balises <tr>, <td>, etc.
    """

    cle_visible = f"{prefixe}_legende_marquages_visible"

    if st.button(
        "ℹ️ Voir / masquer la signification des marquages",
        key=f"{prefixe}_btn_legende_marquages",
        use_container_width=True,
    ):
        st.session_state[cle_visible] = not st.session_state.get(cle_visible, False)
        st.rerun()

    if not st.session_state.get(cle_visible, False):
        return

    st.markdown("### Légende des marquages")

    st.caption(
        "Ces logos permettent de repérer rapidement l'état ou l'importance d'un autre mélange."
    )

    # En-tête
    col_logo_h, col_nom_h, col_desc_h = st.columns([0.7, 2.2, 5])

    col_logo_h.markdown("**Logo**")
    col_nom_h.markdown("**Marquage**")
    col_desc_h.markdown("**Signification**")

    st.markdown("---")

    for option in OPTIONS_MARQUAGES_AUTRES_MELANGES:

        if option.get("code") == "aucun":
            continue

        col_logo, col_nom, col_desc = st.columns([0.7, 2.2, 5])

        col_logo.markdown(f"### {option.get('emoji', '')}")

        col_nom.markdown(
            f"**{option.get('libelle', '-')}**"
        )
        ...
        st.markdown(
            "<hr style='margin:4px 0;opacity:.18;'>",
            unsafe_allow_html=True,
        )

# =============================================================================
# MARQUAGE VISUEL GÉNÉRIQUE : COULEURS, COMPOSANTS, AUTRES MÉLANGES
# =============================================================================

def get_couleur_depuis_objet(objet_data):
    """
    Récupère la couleur de marquage d'un objet :
    couleur, composant, mélange, etc.
    """
    objet_data = objet_data or {}

    return get_option_couleur_autre_melange(
        objet_data.get("couleur_marquage")
        or objet_data.get("couleur_texte")
        or objet_data.get("couleur_texte_id")
    )


def get_marquage_depuis_objet(objet_data):
    """
    Récupère le marquage emoji d'un objet :
    couleur, composant, mélange, etc.
    """
    objet_data = objet_data or {}

    return get_option_marquage_autre_melange(
        code=objet_data.get("marquage_code"),
        emoji=objet_data.get("marquage_emoji"),
        libelle=objet_data.get("marquage_libelle"),
    )


def valeurs_apparence_marquage_objet(couleur_id="aucune", marquage_code="aucun"):
    """
    Retourne les champs prêts à sauvegarder dans l'objet.
    """
    couleur = get_option_couleur_autre_melange(couleur_id)
    marquage = get_option_marquage_autre_melange(code=marquage_code)
...
def afficher_selecteurs_marquage_objet(prefixe, objet_existant=None, titre="Marquage visuel"):
    """
    Sélecteurs couleur + emoji utilisables pour couleurs, composants et autres mélanges.
    """
    objet_existant = objet_existant or {}

    couleur_actuelle = get_couleur_depuis_objet(objet_existant)
    marquage_actuel = get_marquage_depuis_objet(objet_existant)

    ids_couleurs = [option["id"] for option in OPTIONS_COULEURS_AUTRES_MELANGES]
    codes_marquages = [option["code"] for option in OPTIONS_MARQUAGES_AUTRES_MELANGES]

    st.markdown(f"##### {titre}")

    couleur_id = st.selectbox(
        "Couleur du texte dans les tableaux",
        ids_couleurs,
        index=ids_couleurs.index(couleur_actuelle["id"]) if couleur_actuelle["id"] in ids_couleurs else 0,
        format_func=lambda ident: get_option_couleur_autre_melange(ident)["label"],
        key=f"{prefixe}_couleur_marquage",
    )

    couleur_option = get_option_couleur_autre_melange(couleur_id)
    ...
    st.markdown(
        ...
        <div style="
            ...
            border-radius:8px;
            background:#FFFEFB;
            color:{couleur_option['hex']};
            font-weight:900;
        ">
            Aperçu couleur : {html_lib.escape(couleur_option['label'])}
        </div>
        """,
        unsafe_allow_html=True,
    )

    marquage_code = st.selectbox(
        "Logo / marquage",
        codes_marquages,
        index=codes_marquages.index(marquage_actuel["code"]) if marquage_actuel["code"] in codes_marquages else 0,
        format_func=lambda code: (
            f"{get_option_marquage_autre_melange(code=code)['emoji']} "
            f"{get_option_marquage_autre_melange(code=code)['libelle']}"
        ).strip(),
        key=f"{prefixe}_marquage_code",
    )

    marquage_option = get_option_marquage_autre_melange(code=marquage_code)

    if marquage_option["code"] != "aucun":
        st.caption(
            f"{marquage_option['emoji']} {marquage_option['libelle']} : "
            f"{marquage_option['description']}"
        )

    afficher_legende_marquages(f"{prefixe}_legende")

    return valeurs_apparence_marquage_objet(couleur_id, marquage_code)
...
def apparence_ligne_objet_marque(objet_data):
    ...
    if marquage_option["code"] != "aucun":
        prefixe = f"{marquage_option['emoji']} "
        tooltip = f"{marquage_option['libelle']} : {marquage_option['description']}"
        gras = True
...
def selectbox_filtre_couleur_marquage(prefixe, label="Filtrer par couleur de texte"):
    ...
    return st.selectbox(
        label,
        options,
        key=f"{prefixe}_filtre_couleur_marquage",
        format_func=lambda ident: (
            "Toutes les couleurs"
            if ident == "Toutes"
            else get_option_couleur_autre_melange(ident)["label"]
        ),
    )


def selectbox_filtre_emoji_marquage(prefixe, label="Filtrer par marquage"):
    options = ["Tous"] + [option["code"] for option in OPTIONS_MARQUAGES_AUTRES_MELANGES]

    return st.selectbox(
        label,
        options,
        key=f"{prefixe}_filtre_emoji_marquage",
        format_func=lambda code: (
            "Tous les marquages"
            if code == "Tous"
            else (
...
def afficher_selecteurs_apparence_autre_melange(prefixe, melange_data=None):
    """
    Champs de sélection couleur + marquage pour les Autres Mélanges.
    Retourne les valeurs prêtes à sauvegarder dans le mélange.
    """
    melange_data = melange_data or {}

    couleur_actuelle = get_couleur_depuis_melange(melange_data)
    marquage_actuel = get_marquage_depuis_melange(melange_data)

    ids_couleurs = [option["id"] for option in OPTIONS_COULEURS_AUTRES_MELANGES]
    codes_marquages = [option["code"] for option in OPTIONS_MARQUAGES_AUTRES_MELANGES]

    st.markdown("##### Marquage visuel dans le tableau")
...
def apparence_ligne_melange(melange_data):
    ...
    if melange_couleurs_verifie:
        return {
            "couleur": "#CC0605",
            "couleur_secondaire": "#CC0605",
            "prefixe": "🔴 ",
            "tooltip": "Mélange couleur vérifié",
            "gras": True,
        }
    ...
    if categorie_melange != "Mélange Couleurs":
        ...
        return {
            ...
            "prefixe": prefixe,
            "tooltip": tooltip,
            "gras": couleur_option["id"] != "aucune" or marquage_option["code"] != "aucun",
        }
...
def _vider_cles_session(prefixes):
    """Supprime les valeurs de widgets liées à un dialogue fermé."""
    for cle in list(st.session_state.keys()):
        cle_txt = str(cle)
        if any(cle_txt.startswith(prefixe) for prefixe in prefixes):
            st.session_state.pop(cle, None)


def _prefixes_dialogue(type_dialogue):
    """Liste des clés de session à nettoyer selon le type de fenêtre."""
    return {
        "ajout_couleur": [
            "pro_add_color_",
            "input_add_couleur_",
            "new_spec_input_add_couleur",
            "reset_spec_input_add_couleur",
        ],
        "modif_couleur": [
            "pro_edit_color_",
            "input_pro_edit_couleur_",
            "new_spec_input_pro_edit_couleur_",
            "reset_spec_input_pro_edit_couleur_",
            # Nettoie aussi cette ancienne clé utilisée par erreur dans la modif couleur
            "pro_edit_element_cause_dlg_",
        ],
        "ajout_element": [
            "pro_add_element_",
            "input_pro_add_element_",
            "new_spec_input_pro_add_element",
            "reset_spec_input_pro_add_element",
        ],
        "modif_element": [
            "pro_edit_element_",
            "input_pro_edit_element_",
            "new_spec_input_pro_edit_element_",
            "reset_spec_input_pro_edit_element_",
        ],
        "ajout_melange": [
            "pro_add_mix_",
            "input_pro_add_mix_",
            "new_spec_input_pro_add_mix",
            "reset_spec_input_pro_add_mix",
        ],
        "modif_melange": [
            "pro_edit_mix_",
            "input_pro_edit_mix_",
            "new_spec_input_pro_edit_mix_",
            "reset_spec_input_pro_edit_mix_",
        ],
        "edit_trash_Couleur": [
            "trash_edit_color_",
        ],
...
def nettoyer_state_dialogue(type_dialogue):
    """Nettoie les champs temporaires du dialogue."""
    prefixes = _prefixes_dialogue(type_dialogue)
    if prefixes:
        _vider_cles_session(prefixes)


def fermer_dialogue_actif():
    """
    Ferme réellement le dialogue actif sans récursion.

    Important :
    - ne pas appeler fermer_dialogue_actif() dans cette fonction ;
    - ne pas mettre st.rerun() ici ;
    - cette fonction peut être utilisée dans on_dismiss.
    """

    if st.session_state.get("_fermeture_dialogue_en_cours", False):
        return

    st.session_state["_fermeture_dialogue_en_cours"] = True

    try:
        dialog = st.session_state.get("dialog_actif")
        type_dialogue = dialog.get("type") if isinstance(dialog, dict) else None

        st.session_state.pop("dialog_actif", None)
        st.session_state.pop("active_dialog", None)

        if type_dialogue:
            nettoyer_state_dialogue(type_dialogue)

    finally:
        st.session_state["_fermeture_dialogue_en_cours"] = False

def ouvrir_dialogue_actif(type_dialogue, index=None):
    """
    Ouvre un dialogue proprement.

    Ça évite de rouvrir une ancienne fenêtre avec des valeurs non enregistrées.
    """
    ancien_dialogue = st.session_state.get("dialog_actif")
    if isinstance(ancien_dialogue, dict):
        nettoyer_state_dialogue(ancien_dialogue.get("type"))
    ...
    payload = {"type": type_dialogue}
    if index is not None:
        payload["index"] = index

    st.session_state.dialog_actif = payload
...
def generer_fichier_export(donnees_list, nom_fichier="export"):
    ...
    if "Couleurs" in nom_fichier:
        colonnes_map = {
            ...
            "societe": "Société",
            "type": "Type",
            "couleur_texte_libelle": "Couleur de texte",
            "marquage_emoji": "Logo",
            "marquage_libelle": "Marquage",
            "marquage_description": "Signification marquage",            
        }
...
def generer_pdf_fiche_technique(data_obj, type_ft="melange"):
    ...
    def cellule_composant_avec_pastille(nom_composant, couleur_hex=None, afficher_pastille=False):
        """
        Crée la cellule PDF du nom de composant.
        Si le composant est une couleur / RAL, ajoute une petite pastille colorée.
        """
        nom_pdf = nettoyer_valeur_pdf(nom_composant or "-")
        couleur_hex = str(couleur_hex or "").strip()

        if not afficher_pastille or not re.fullmatch(r"#[0-9a-fA-F]{6}", couleur_hex):
            return Paragraph(nom_pdf, body_style)

        couleur_hex = couleur_hex.upper()

        try:
            from reportlab.graphics.shapes import Drawing as RLDrawing, Circle

            pastille = RLDrawing(10, 10)
            pastille.add(
                Circle(
                    5,
                    5,
                    4.2,
                    fillColor=colors.HexColor(couleur_hex),
                    strokeColor=colors.HexColor("#142235"),
                    strokeWidth=0.45,
                )
            )

            cellule = Table(
                [[pastille, Paragraph(nom_pdf, body_style)]],
                colWidths=[12, 78],
            )
            ...
            # Sécurité : si Drawing/Circle pose problème, on affiche quand même une pastille texte.
            return Paragraph(
                f"<font color='{couleur_hex}'>●</font>&#160;{nom_pdf}",
                body_style,
            )
        
    def recuperer_images_commentaires_ft(data_obj, ft_data):
        """
        Récupère les images associées aux commentaires généraux.
        Compatible avec stockage top-level ou dans fiche_technique.
        """
        images = (
            data_obj.get("commentaire_global_images")
            or data_obj.get("images_commentaire_global")
            or ft_data.get("commentaire_global_images")
            or []
        )
        ...
        return [
            ...
            if isinstance(image_info, dict) and image_info.get("data")
        ]


    def creer_image_commentaire_pdf(image_info, max_width=125, max_height=85):
        """
        Convertit une image base64 en RLImage redimensionnée pour le PDF.
        """
        try:
            image_bytes = base64.b64decode(image_info.get("data", ""))
            ...
            ratio = min(max_width / largeur, max_height / hauteur, 1)

            largeur_pdf = largeur * ratio
            hauteur_pdf = hauteur * ratio

            buffer_img = io.BytesIO()
            image_pil.save(buffer_img, format="PNG")
            buffer_img.seek(0)
            ...
            return None    
    def texte_simple_pdf(valeur):
        return str(valeur or "").strip()
    
    def texte_normalise_pdf(valeur):
        return texte_simple_pdf(valeur).lower()
    
    def trouver_element_pour_composant(composant, tous_elements):
        """
        Retrouve un composant dans le catalogue des éléments.
        Recherche par nom, ref, code ou désignation.
        """
        valeurs_composant = {
            texte_normalise_pdf(composant.get("ref")),
            texte_normalise_pdf(composant.get("nom")),
            texte_normalise_pdf(composant.get("code")),
            texte_normalise_pdf(composant.get("designation")),
        }
        ...
        for element in tous_elements:
            valeurs_element = {
                texte_normalise_pdf(element.get("nom")),
                texte_normalise_pdf(element.get("code")),
                texte_normalise_pdf(element.get("designation")),
                texte_normalise_pdf(element.get("designation_achat")),
            }
    
            valeurs_element.discard("")
    
            if valeurs_composant & valeurs_element:
                return element
    
        return None
    
    def cause_danger_element(item, composant=None):
        """
        Retourne la cause du danger.
        Si rien n'est précisé, retourne le texte demandé.
        """
        composant = composant or {}
    
        cause = texte_simple_pdf(
            (item or {}).get("danger_texte")
            or composant.get("danger_texte")
            or composant.get("cause_danger")
        )
    
        if not cause:
            cause = "voir les données du composant"
    ...
    # -----------------------------------------------------------------------
    if type_ft == "melange":
        titre_principal = "FICHE TECHNIQUE MELANGE"
    elif type_ft == "couleur":
        titre_principal = "FICHE TECHNIQUE COULEUR"
    else:
        titre_principal = "FICHE TECHNIQUE COMPOSANT"
    ...
    if type_ft == "melange":
        ...
        if data_obj.get("statut") == "Vérifié":
            titre_melange += " <font color='#CC0605'>🔴 (Vérifié)</font>"
        infos_rows = [
        ...
        # Marquage visuel des Autres Mélanges dans la FT
        if data_obj.get("categorie_choisie") != "Mélange Couleurs":

            couleur_info = get_couleur_depuis_melange(data_obj)
            marquage_info = get_marquage_depuis_melange(data_obj)

            if marquage_info["code"] != "aucun":
                texte_marquage_pdf = (
                    f"{marquage_info['emoji']} {marquage_info['libelle']} — "
                    f"{marquage_info['description']}"
                )

                infos_rows.append(
                    [
                        Paragraph("<b>Marquage :</b>", body_style),
                        Paragraph(nettoyer_valeur_pdf(texte_marquage_pdf), body_style),
                    ]
                )

            if couleur_info["id"] != "aucune":
                couleur_hex_pdf = couleur_info["hex"]
                couleur_label_pdf = nettoyer_valeur_pdf(couleur_info["label"])

                infos_rows.append(
                    [
                        Paragraph("<b>Couleur d'affichage :</b>", body_style),
                        Paragraph(
                            f"<font color='{couleur_hex_pdf}'>{couleur_label_pdf}</font>",
                            body_style,
                        ),
                    ]
                )        
        ...
        if data_obj.get("statut") == "Vérifié":
            titre_element += " <font color='#CC0605'>🔴 (Vérifié)</font>"
        infos_rows = [
    ...
    # -----------------------------------------------------------------------
    # Marquage visuel dans la FT : couleurs et composants
    # -----------------------------------------------------------------------
    if type_ft != "melange":

        couleur_info = get_couleur_depuis_objet(data_obj)
        marquage_info = get_marquage_depuis_objet(data_obj)
    ...
    # =======================================================================
    # RISQUE / DANGER — Composant uniquement
    # =======================================================================
    if type_ft not in ["melange", "couleur"]:
        danger_element = str(data_obj.get("danger", "") or "").strip()
        if danger_element.lower() == "oui":   
            cause_element = str(data_obj.get("danger_texte", "") or "").strip()    
            if not cause_element:    
                cause_element = "voir les données du composant"    
            story.append(Paragraph("Risque / Danger du composant", h2_style))    
            data_danger = [
                [
                    Paragraph("<b>Nom de l'élément à risque</b>", body_bold),
                    Paragraph("<b>Causes de danger</b>", body_bold),
                ],
                [
                    Paragraph(
                        nettoyer_valeur_pdf(data_obj.get("nom", "-")),
                        body_style,
                    ),
                    Paragraph(
                        nettoyer_valeur_pdf(cause_element),
                        body_style,
                    ),
                ],
            ]
            ...
            t_danger.setStyle(
                TableStyle(
                    [
                        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#FCEBEC")),
                        ("TEXTCOLOR", (0, 0), (-1, 0), colors.HexColor("#66202E")),
                        ("PADDING", (0, 0), (-1, -1), 5),
                        ("GRID", (0, 0), (-1, -1), 0.5, colors.HexColor("#E2A3A7")),
                        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
                        ("BACKGROUND", (0, 1), (-1, -1), colors.HexColor("#FFF7F7")),
                    ]
                )
            )
            story.append(t_danger)
            story.append(Spacer(1, 1))
    ...
    if type_ft == "melange":
        ...
        elements_a_risque = []
        elements_a_risque_deja_ajoutes = set()
        
        for c in data_obj.get("couleurs_associees", []):

            code_aff, des_aff, dan_aff = "-", "-", "-"
            couleur_pastille = None
            afficher_pastille = False

            ctype = c.get("type")

            if ctype in ["additif", "element"]:
            
                item = trouver_element_pour_composant(c, tous_elements)
                ...
                if item:
                    ...
                    danger_declare = texte_normalise_pdf(item.get("danger")) == "oui"
                    ...
                    dan_aff = c.get("danger", "-")
            
                    danger_declare = texte_normalise_pdf(c.get("danger")) == "oui"
            
                if danger_declare:
            
                    source_risque = item if item else c
            
                    nom_element_risque = (
            
                        source_risque.get("nom")
                    ...
                    cause_danger = cause_danger_element(item, c)
            
                    cle_risque = str(nom_element_risque).strip().lower()
            
                    if cle_risque not in elements_a_risque_deja_ajoutes:
            
                        elements_a_risque.append(
            
                            {
            
                                "nom": nom_element_risque,
                        ...
                        elements_a_risque_deja_ajoutes.add(cle_risque)


            elif ctype == "couleur":

                afficher_pastille = True
                couleur_pastille = c.get("visuel")

                item = next(
                    (a for a in toutes_couleurs if a.get("ref") == c.get("ref")),
                    None,
                )
                ...
                if item:
                    ...
                    couleur_pastille = item.get("visuel") or couleur_pastille

            elif ctype == "melange_base":

                item = next(
                    (a for a in tous_melanges if a.get("ref") == c.get("ref")),
                    None,
                )
                ...
                numero_ral_pdf = "".join(filter(str.isdigit, str(c.get("ref", ""))))
                couleur_pastille = c.get("visuel") or RAL_DICT.get(numero_ral_pdf)
            ...
            data_comp.append(
                [
                    ...
                    cellule_composant_avec_pastille(
                        c.get("nom", "-"),
                        couleur_hex=couleur_pastille,
                        afficher_pastille=afficher_pastille,
                    ),
        ...
        # -------------------------------------------------------------------
        # TABLEAU DES ÉLÉMENTS À RISQUE / DANGER
        # -------------------------------------------------------------------
        if elements_a_risque:

            story.append(Spacer(1, 1))

            story.append(Paragraph("Éléments à risque / danger déclarés", h2_style))

            data_risques = [
                [
                    Paragraph("<b>Nom de l'élément à risque</b>", body_bold),
                    Paragraph("<b>Explications</b>", body_bold),
                ]
            ]

            for element_risque in elements_a_risque:
                data_risques.append(
                    [
                        Paragraph(
                            nettoyer_valeur_pdf(element_risque.get("nom", "-")),
                            body_style,
                        ),
                        Paragraph(
                            nettoyer_valeur_pdf(
    ...
    t_comm = Table(
        [[Paragraph(nettoyer_valeur_pdf(comm_global), body_style)]],
        colWidths=[512],
    )
    ...
    story.append(t_comm)
    # -----------------------------------------------------------------------
    # Images liées aux commentaires généraux
    # -----------------------------------------------------------------------
    images_commentaires = recuperer_images_commentaires_ft(data_obj, ft_data)
    if images_commentaires:
        story.append(Spacer(1, 5))
        caption_img_style = ParagraphStyle(
            "CaptionImageCommentaire",
        ...
        cellules_images = []
        for image_info in images_commentaires:
            image_pdf = creer_image_commentaire_pdf(image_info)
            if image_pdf is None:
                continue
            nom_image = nettoyer_valeur_pdf(image_info.get("name", "Image commentaire"))
            cellules_images.append(
                [
                    image_pdf,
                    Paragraph(nom_image, caption_img_style),
                ]
            )
        if cellules_images:
            story.append(
                Paragraph("Images associées aux commentaires généraux", body_bold)
            )
            # Affiche les images par lignes de 3
            for index_img in range(0, len(cellules_images), 3):
                ligne_images = cellules_images[index_img:index_img + 3]
                while len(ligne_images) < 3:
                    ligne_images.append("")
                t_images = Table(
                    [ligne_images],
                    colWidths=[170, 170, 172],
                )
                t_images.setStyle(
                    TableStyle(
                        [
    ...
    redacteur_signature = ft_data.get("redacteur", "Labo / R&D")

    def canvasmaker_ft(*args, **kwargs):
        return NumeroteurDePages(
            *args,
            signature_redacteur=redacteur_signature,
            signature_direction="Responsable Qualité",
            afficher_signature_derniere_page=True,
            **kwargs,
        )

    doc.build(
        story,
        onFirstPage=dessiner_fond_ivoire_et_entete,
        onLaterPages=dessiner_fond_ivoire_et_entete,
        canvasmaker=canvasmaker_ft,
    )
...
# =============================================================================
# APERÇU PDF DANS UNE FENÊTRE STREAMLIT
# =============================================================================

def pdf_en_octets(pdf_obj):
    """
    Convertit le PDF généré en bytes.
    Compatible avec BytesIO, bytes, bytearray.
    """
    if pdf_obj is None:
        return b""

    if isinstance(pdf_obj, bytes):
        return pdf_obj

    if isinstance(pdf_obj, bytearray):
...
def cle_streamlit_pdf(valeur):
    """
    Nettoie une valeur pour créer une clé Streamlit stable.
    """
    return re.sub(r"[^a-zA-Z0-9_]+", "_", str(valeur or ""))[:90]


def afficher_apercu_pdf(pdf_bytes, hauteur=760):
    """
    Affiche un PDF directement dans la fenêtre Streamlit.
    """
    if not pdf_bytes:
        st.error("Aperçu PDF impossible : PDF vide.")
        return
    ...
    <body>
        ...
        <div class="message">
            Si l’aperçu ne s’affiche pas, utilisez le bouton Télécharger.
        </div>
    </body>
    </html>
    """
...
def ouvrir_visualisation_ft(melange):
    ...
    nom_fichier = f"FT_{nom_p}_{ref_p}.pdf".replace(" ", "_").replace("/", "-")

    pdf_bytes = pdf_en_octets(
        generer_pdf_fiche_technique(melange, "melange")
    )

    cle_pdf = cle_streamlit_pdf(f"melange_{ref_p}_{nom_p}")
    cle_preview = f"preview_pdf_{cle_pdf}"
    ...
    with col_actions:
        ...
        col_visualiser, col_telecharger, col_masquer = st.columns(3)
        ...
        with col_visualiser:
            if st.button(
                ...
                type="primary",
                use_container_width=True,
                key=f"btn_preview_{cle_pdf}",
            ):
                st.session_state[cle_preview] = True
        ...
        with col_telecharger:
            st.download_button(
                ...
                use_container_width=True,
                key=f"btn_download_{cle_pdf}",
            )

        with col_masquer:
            if st.session_state.get(cle_preview, False):
                if st.button(
                    "Masquer",
                    use_container_width=True,
                    key=f"btn_hide_preview_{cle_pdf}",
                ):
                    st.session_state[cle_preview] = False
                    st.rerun()
            else:
                st.write("")

        st.caption(
            "Visualiser affiche la fiche dans cette fenêtre. "
            "Télécharger enregistre le PDF sur votre ordinateur."
        )

    if st.session_state.get(cle_preview, False):
        st.markdown("---")
        st.markdown("#### Aperçu de la Fiche Technique")
        afficher_apercu_pdf(pdf_bytes, hauteur=760)
...
def ouvrir_visualisation_ft_additif(additif):
    ...
    nom_fichier = f"FT_{nom_p}_{code_p}.pdf".replace(" ", "_").replace("/", "-")

    pdf_bytes = pdf_en_octets(
        generer_pdf_fiche_technique(additif, "additif")
    )

    cle_pdf = cle_streamlit_pdf(f"element_{code_p}_{nom_p}")
    cle_preview = f"preview_pdf_{cle_pdf}"
...
def ouvrir_visualisation_ft_couleur(couleur):
    ...
    pdf_bytes = pdf_en_octets(
        generer_pdf_fiche_technique(couleur, "couleur")
    )

    cle_pdf = cle_streamlit_pdf(f"couleur_{ref_p}_{nom_p}")
    cle_preview = f"preview_pdf_{cle_pdf}"
    ...
    with col_apercu:
        st.markdown(
            ...
            <div style="
                width:120px;
                height:120px;
                background:{couleur.get('visuel', '#D8DEE4')};
...
def ouvrir_fenetre_aide():
    ...
    with onglets[1]:
        ...
        with st.expander("🎨 Nuancier de couleurs", expanded=True):
            ...
            - **Vérification automatique RAL :** Saisissez un code (ex: 3020) et cliquez sur "Vérifié RAL" pour appliquer la teinte hexadécimale officielle.
...
if G_ACTIF == "preparation_melanges":
    ...
    if choix_section == "🎨 Nuancier de couleurs":
        ...
        nombre_couleurs_verifiees = sum(
            1 for item in liste_couleurs if item.get("statut") == "Vérifié"
        )
        ...
        pro_entete_section(
            ...
            [
                ...
                ("Vérifiés", nombre_couleurs_verifiees),
        ...
        @st.dialog("Ajouter une nouvelle couleur", width="large", dismissible=True, on_dismiss=fermer_dialogue_actif)
        def ouvrir_ajout_couleur():
            st.markdown("### Nouvelle référence couleur")
            apparence_couleur = valeurs_apparence_marquage_objet()
            ...
            with onglet_visuel:
                ...
                apparence_couleur = afficher_selecteurs_marquage_objet(
                    "pro_add_color",
                    {},
                    titre="Marquage visuel du nuancier",
                )                    
                with col_visuel_2:
            ...
            with onglet_ft:
                ...
                if activer_ft:
                    ...
                    images_commentaire_global = gerer_images_commentaire_ft(
                        "pro_add_color",
                        {},
                    )
                ...
                liste_couleurs.append(
                    {
                        ...
                        "statut": c_statut,
                        "commentaire_global_images": images_commentaire_global,
                        "commentaire": commentaire_court.strip(),
                        "couleur_marquage": apparence_couleur["couleur_marquage"],
                        "couleur_texte": apparence_couleur["couleur_texte"],
                        "couleur_texte_libelle": apparence_couleur["couleur_texte_libelle"],
                        "marquage_code": apparence_couleur["marquage_code"],
                        "marquage_emoji": apparence_couleur["marquage_emoji"],
                        "marquage_libelle": apparence_couleur["marquage_libelle"],
                        "marquage_description": apparence_couleur["marquage_description"],                        
        ...
        @st.dialog("Modifier une couleur", width="large", dismissible=True, on_dismiss=fermer_dialogue_actif)
        def ouvrir_modif_couleur(index, couleur_data):
            ...
            with onglet_ft:
                ...
                images_commentaire_global = couleur_data.get("commentaire_global_images", [])
                m_cause_modification = ""
                if activer_ft:
        ...
        with barre_action_1:
            if st.button(
                ...
                ouvrir_dialogue_actif("ajout_couleur")
                st.rerun()
        ...
        with st.expander("Filtres et organisation du nuancier", expanded=True):
            ...
            with filtre_col_4:
                filtre_statut_couleur = st.selectbox(
                    "Statut",
                    ["Tous", "Vérifié", "Pas vérifié"],
                    key="pro_color_filter_status",
                )
            st.markdown("##### Marquage visuel")

            afficher_legende_marquages("pro_color_table")

            filtre_marq_col_1, filtre_marq_col_2 = st.columns(2)

            with filtre_marq_col_1:
                filtre_couleur_marquage_couleur = selectbox_filtre_couleur_marquage(
                    "pro_color",
                    "Filtrer par couleur de texte",
                )

            with filtre_marq_col_2:
                filtre_marquage_couleur = selectbox_filtre_emoji_marquage(
                    "pro_color",
                    "Filtrer par marquage",
                )
        ...
        for index_couleur, couleur_item in enumerate(liste_couleurs):
            ...
            if filtre_statut_couleur != "Tous" and couleur_item.get("statut", "Pas vérifié") != filtre_statut_couleur:
                continue
            couleur_marquage_item = get_couleur_depuis_objet(couleur_item)
            marquage_item = get_marquage_depuis_objet(couleur_item)

            if (
                filtre_couleur_marquage_couleur != "Toutes"
                and couleur_marquage_item["id"] != filtre_couleur_marquage_couleur
            ):
                continue

            if (
                filtre_marquage_couleur != "Tous"
                and marquage_item["code"] != filtre_marquage_couleur
            ):
                continue            
        ...
        if not couleurs_page:
            ...
            for couleur_ligne in couleurs_page:
                ...
                style_couleur_ligne = apparence_ligne_objet_marque(couleur_data)

                couleur_texte_ligne = style_couleur_ligne["couleur"]
                prefixe_ligne = style_couleur_ligne["prefixe"]
                tooltip_ligne = html_lib.escape(style_couleur_ligne.get("tooltip", ""))
                font_weight_ligne = "800" if style_couleur_ligne.get("gras") else "400"

                ref_txt = html_lib.escape(pro_texte(couleur_data.get("ref")) or "-")
                nom_actuel_txt = html_lib.escape(pro_texte(couleur_data.get("nom_actuel")) or "-")
                nom_futur_txt = html_lib.escape(pro_texte(couleur_data.get("nom_futur")) or "-")
                ral_txt = html_lib.escape(pro_texte(couleur_data.get("ral")) or "-")
                societe_txt = html_lib.escape(pro_texte(couleur_data.get("societe")) or "-")
                type_txt = html_lib.escape(pro_texte(couleur_data.get("type")) or "-")

                ligne[0].markdown(
                    f'<span title="{tooltip_ligne}" style="color:{couleur_texte_ligne};font-weight:800;">{prefixe_ligne}{ref_txt}</span>',
                    unsafe_allow_html=True,
                )

                ligne[1].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{nom_actuel_txt}</span>',
                    unsafe_allow_html=True,
                )

                ligne[2].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{nom_futur_txt}</span>',
                    unsafe_allow_html=True,
                )

                ligne[3].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{ral_txt}</span>',
                    unsafe_allow_html=True,
                )

                ligne[4].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{societe_txt}</span>',
                    unsafe_allow_html=True,
                )

                ligne[5].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{type_txt}</span>',
                    unsafe_allow_html=True,
                )
                ligne[6].markdown(
                ...
                if action_modifier.button("✎", key=f"pro_color_edit_{index_couleur}"):
                    ouvrir_dialogue_actif("modif_couleur", index_couleur)
                    st.rerun()
    ...
    # ========================================================================
        ...
        nombre_elements_verifies = sum(
            1 for item in liste_additifs if item.get("statut") == "Vérifié"
        )
        ...
        @st.dialog("Ajouter un élément", width="large", dismissible=True, on_dismiss=fermer_dialogue_actif)
        def ouvrir_ajout_element_complet():
            st.markdown("### Nouvelle matière / nouvel élément")
            apparence_element = valeurs_apparence_marquage_objet()
            ...
            with onglet_identite:
                ...
                with col_identite_2:
                    e_comm = st.text_input(
                        "Commentaire court (N'apparait pas dans la FT)",
            ...
            with onglet_classement:
                ...
                apparence_element = afficher_selecteurs_marquage_objet(
                    "pro_add_element",
                    {},
                    titre="Marquage visuel du composant",
                )
            with onglet_ft:
                activer_ft = st.toggle(
                    "Créer la Fiche Technique (FT)",
                ...
                if activer_ft:
                    ...
                    e_images_commentaire_global = gerer_images_commentaire_ft(
                        "pro_add_element",
                        {},
                    )
                ...
                liste_additifs.append(
                    {
                        ...
                        "couleur_marquage": apparence_element["couleur_marquage"],
                        "couleur_texte": apparence_element["couleur_texte"],
                        "couleur_texte_libelle": apparence_element["couleur_texte_libelle"],
                        "marquage_code": apparence_element["marquage_code"],
                        "marquage_emoji": apparence_element["marquage_emoji"],
                        "marquage_libelle": apparence_element["marquage_libelle"],
                        "marquage_description": apparence_element["marquage_description"],                        
                        "commentaire_global": e_commentaire_global.strip(),
                        "commentaire_global_images": e_images_commentaire_global,
                        "has_ft": activer_ft,
                        "fiche_technique": {
                            "specs_dynamiques": specs_dynamiques,
                            "date_creation": date_creation,
        ...
        @st.dialog("Modifier un élément", width="large", dismissible=True, on_dismiss=fermer_dialogue_actif)
        def ouvrir_modif_element_complet(index, data):
            ...
            with onglet_ft:
                ...
                m_images_commentaire_global = data.get("commentaire_global_images", [])
                ...
                if activer_ft:
                    ...
                    m_images_commentaire_global = gerer_images_commentaire_ft(
                        f"pro_edit_element_{index}",
                        data,
                    )
                    m_cause_modification = st.text_input(
                ...
                element_modifie.update(
                    {
                        ...
                        "commentaire_global_images": m_images_commentaire_global,
                        "has_ft": activer_ft,
                        "fiche_technique": {
        ...
        # --------------------------------------------------------------------
        # GESTION DES COMPARTIMENTS ET SOUS-GROUPES DU CATALOGUE DES COMPOSANTS
        # --------------------------------------------------------------------
        with st.expander("⚙️ Gérer / Supprimer les compartiments du catalogue", expanded=False):

            st.markdown("### Compartiments existants")

            col_add_comp, col_btn_comp = st.columns([3, 1])

            with col_add_comp:
                nouveau_compartiment_catalogue = st.text_input(
                    "Créer un nouveau compartiment",
                    key="input_new_compartiment_catalogue",
                    placeholder="Exemple : Résine, Peinture, Moulage..."
                )
            ...
            with col_btn_comp:
                ...
                if st.button(
                    "➕ Ajouter",
                    key="btn_add_compartiment_catalogue",
                    use_container_width=True,
                ):
                    comp_clean = majuscule_initiale(nouveau_compartiment_catalogue)

                    if not comp_clean:
                        st.warning("Veuillez saisir un nom de compartiment.")

                    elif any(
                        pro_texte(comp).lower() == comp_clean.lower()
                        for comp in liste_compartiments
                    ):
                        st.warning("Ce compartiment existe déjà.")

                    else:
                        liste_compartiments.append(comp_clean)
                        sauvegarder_donnees()
                        st.rerun()
            ...
            if not liste_compartiments:
                ...
                for idx_comp, comp_nom in enumerate(list(liste_compartiments)):
                    ...
                    col_nom_comp, col_nb_comp, col_del_comp = st.columns([3, 1, 1])

                    col_nom_comp.write(f"• **{comp_nom}**")
                    col_nb_comp.caption(f"{nb_utilisations} élément(s)")

                    if col_del_comp.button(
                        "🗑️",
            ...
            st.markdown("---")
            # ---------------------------------------------------------------
            # RENOMMER UN COMPARTIMENT DU CATALOGUE
            # ---------------------------------------------------------------
            st.markdown("### Renommer un compartiment")

            if liste_compartiments:

                col_old_comp, col_new_comp, col_btn_rename_comp = st.columns([2, 2, 1])

                with col_old_comp:
                    ancien_compartiment = st.selectbox(
                        "Compartiment à renommer",
                        liste_compartiments,
                        key="select_rename_compartiment_catalogue",
                    )

                with col_new_comp:
                    nouveau_nom_compartiment = st.text_input(
                        "Nouveau nom du compartiment",
                        key="input_rename_compartiment_catalogue",
                        placeholder="Exemple : Résine souple",
                    )

                with col_btn_rename_comp:
                    st.write("")
                    st.write("")

                    if st.button(
                        "✏️ Renommer",
                        key="btn_rename_compartiment_catalogue",
                        use_container_width=True,
                    ):
                        nouveau_clean = nettoyer_nom_compartiment(nouveau_nom_compartiment)

                        if not ancien_compartiment:
                            st.warning("Aucun compartiment sélectionné.")

                        elif not nouveau_clean:
                            st.warning("Veuillez saisir un nouveau nom.")

                        elif normaliser_nom_compartiment(ancien_compartiment) == normaliser_nom_compartiment(nouveau_clean):
                            # Permet quand même de corriger l'affichage, par exemple résine -> Résine
                            renommer_compartiment_composants(
                                liste_compartiments,
                                liste_additifs,
                                ancien_compartiment,
                                nouveau_clean,
                        ...
                        elif nom_existe_deja_dans_liste(
                            liste_compartiments,
                            nouveau_clean,
                            ancien_compartiment,
                        ):
                            st.error("Ce nom de compartiment existe déjà. Choisissez un autre nom.")
                            ...
                            sauvegarder_donnees()
                            afficher_animation_validation("Compartiment renommé")
                            st.rerun()

            else:
                st.info("Aucun compartiment à renommer.")
            # ----------------------------------------------------------------
            # SOUS-GROUPES
            # ----------------------------------------------------------------
            st.markdown("### Sous-groupes existants")

            if "Général" not in liste_sous_groupes:
                liste_sous_groupes.insert(0, "Général")

            col_add_sg, col_btn_sg = st.columns([3, 1])

            with col_add_sg:
                nouveau_sous_groupe_catalogue = st.text_input(
                    "Créer un nouveau sous-groupe",
                    key="input_new_sous_groupe_catalogue",
                    placeholder="Exemple : Pigments, Durcisseurs, Charges..."
                )
            ...
            with col_btn_sg:
                ...
                if st.button(
                    "➕ Ajouter",
                    key="btn_add_sous_groupe_catalogue",
                    use_container_width=True,
                ):
                    sg_clean = majuscule_initiale(nouveau_sous_groupe_catalogue)

                    if not sg_clean:
                        st.warning("Veuillez saisir un nom de sous-groupe.")

                    elif any(
                        pro_texte(sg).lower() == sg_clean.lower()
                        for sg in liste_sous_groupes
                    ):
                        st.warning("Ce sous-groupe existe déjà.")

                    else:
                        liste_sous_groupes.append(sg_clean)
                        sauvegarder_donnees()
                        st.rerun()

            st.markdown("**Liste des sous-groupes :**")

            for idx_sg, sg_nom in enumerate(list(liste_sous_groupes)):
        ...
        with st.expander("Filtres et organisation du catalogue", expanded=True):
            ...
            afficher_legende_marquages("pro_element_table")

            filtre_element_marq_1, filtre_element_marq_2 = st.columns(2)

            with filtre_element_marq_1:
                filtre_couleur_marquage_element = selectbox_filtre_couleur_marquage(
                    "pro_element",
                    "Filtrer par couleur de texte",
                )

            with filtre_element_marq_2:
                filtre_marquage_element = selectbox_filtre_emoji_marquage(
                    "pro_element",
                    "Filtrer par marquage",
                )
        ...
        for index_element, element_item in enumerate(liste_additifs):
            ...
            if filtre_statut_element != "Tous" and element_item.get("statut", "Pas vérifié") != filtre_statut_element:
                continue
            couleur_marquage_item = get_couleur_depuis_objet(element_item)
            marquage_item = get_marquage_depuis_objet(element_item)

            if (
                filtre_couleur_marquage_element != "Toutes"
                and couleur_marquage_item["id"] != filtre_couleur_marquage_element
            ):
                continue

            if (
                filtre_marquage_element != "Tous"
                and marquage_item["code"] != filtre_marquage_element
            ):
                continue            
        ...
        def pro_afficher_tableau_elements(liste_tableau, prefixe_tableau):
            ...
            for element_ligne in elements_page:
                ...
                style_element_ligne = apparence_ligne_objet_marque(element_data)

                couleur_texte_ligne = style_element_ligne["couleur"]
                prefixe_marquage = style_element_ligne["prefixe"]
                tooltip_ligne = html_lib.escape(style_element_ligne.get("tooltip", ""))
                font_weight_ligne = "800" if style_element_ligne.get("gras") else "400"

                indicateur_danger = "🔴 " if element_data.get("danger") == "Oui" else ""

                nom_txt = html_lib.escape(pro_texte(element_data.get("nom")) or "-")
                code_txt = html_lib.escape(pro_texte(element_data.get("code")) or "-")
                designation_txt = html_lib.escape(pro_texte(element_data.get("designation")) or "-")
                fournisseur_txt = html_lib.escape(pro_texte(element_data.get("fournisseur")) or "-")
                compartiments_txt = html_lib.escape(", ".join(element_data.get("compartiments", [])) or "-")
                commentaire_txt = html_lib.escape(pro_texte(element_data.get("commentaire")) or "-")

                ligne[0].markdown(
                    f'<span title="{tooltip_ligne}" style="color:{couleur_texte_ligne};font-weight:800;">{indicateur_danger}{prefixe_marquage}{nom_txt}</span>',
                    unsafe_allow_html=True,
                )

                ligne[1].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{code_txt}</span>',
                    unsafe_allow_html=True,
                )

                ligne[2].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{designation_txt}</span>',
                    unsafe_allow_html=True,
                )

                ligne[3].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{fournisseur_txt}</span>',
                    unsafe_allow_html=True,
                )

                ligne[4].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{compartiments_txt}</span>',
                    unsafe_allow_html=True,
                )

                ligne[5].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{commentaire_txt}</span>',
                    unsafe_allow_html=True,
                )
        ...
        noms_onglets_elements = ["Tous les composants"] + [
            f"{majuscule_initiale(compartiment)}"
        ...
        if isinstance(dialog_actif, dict):
            d_type = dialog_actif.get("type")
            if d_type == "ajout_element":
                ouvrir_ajout_element_complet()
            elif d_type == "modif_element":
                idx = dialog_actif.get("index")
                if idx is not None and 0 <= idx < len(liste_additifs):
                    ouvrir_modif_element_complet(idx, liste_additifs[idx])
    ...
    # ========================================================================
        ...
        nombre_melanges_verifies = sum(
            1 for item in liste_melanges if item.get("statut") == "Vérifié"
        )
        ...
        def pro_editeur_composants(cle_etat, prefixe_widgets, reference_exclue=None):
            ...
            cle_retraits_forces = f"{prefixe_widgets}_retraits_forces"
        
            for uid_retire in st.session_state.pop(cle_retraits_forces, []):
        
                etat_composants.pop(uid_retire, None)
        
                empreinte_retiree = pro_cle_composant_widget(uid_retire)
        
                # Décocher la case du composant
                st.session_state[f"{prefixe_widgets}_candidate_{empreinte_retiree}"] = False
        
                # Nettoyer aussi les anciens champs dosage / note du composant retiré
                for cle_state in list(st.session_state.keys()):
                    cle_txt = str(cle_state)
        
                    if (
                        empreinte_retiree in cle_txt
                        and (
                            cle_txt.startswith(f"{prefixe_widgets}_dosage_")
                            or cle_txt.startswith(f"{prefixe_widgets}_note_")
                        )
                    ):
                        st.session_state.pop(cle_state, None)
                    ...
                    with st.expander(
                        ...
                        with col_retrait:
                            if st.button(
                                ...
                                etat_composants.pop(uid, None)
                                st.session_state.setdefault(cle_retraits_forces, [])
                            
                                if uid not in st.session_state[cle_retraits_forces]:
                                    st.session_state[cle_retraits_forces].append(uid)
                                st.rerun()
        ...
        @st.dialog("Créer un mélange", width="large", dismissible=True, on_dismiss=fermer_dialogue_actif)
        def ouvrir_ajout_melange_complet():
            apparence_autre_melange = valeurs_apparence_autre_melange()
            ...
            with onglet_general:
                ...
                with col_general_2:
                    ...
                    # Les champs Ateliers ne concernent PAS les Mélanges Couleurs.
                    # Ils apparaissent uniquement pour les Autres Mélanges.
                    if m_categorie != "Mélange Couleurs":
                        ...
                        apparence_autre_melange = afficher_selecteurs_apparence_autre_melange(
                            "pro_add_mix",
                            {},
                        )
                    else:
                    ...
                    # L'option 'Vérifié' s'affiche uniquement pour les Mélanges Couleurs
                    if m_categorie == "Mélange Couleurs":
                        m_statut = st.radio(
                            "État du mélange",
                            ["Pas vérifié", "Vérifié"],
            ...
            if st.button(
                ...
                # Traitement des ateliers uniquement pour les Autres Mélanges.
                # Les Mélanges Couleurs n'ont pas de compartiment atelier.
                if m_categorie != "Mélange Couleurs":
                ...
                liste_melanges.append(
                    {
                        ...
                        "statut": m_statut,
                        "has_ft": activer_ft,
                        "couleur_marquage": apparence_autre_melange["couleur_marquage"],
                        "couleur_texte": apparence_autre_melange["couleur_texte"],
                        "couleur_texte_libelle": apparence_autre_melange["couleur_texte_libelle"],
                        "marquage_code": apparence_autre_melange["marquage_code"],
                        "marquage_emoji": apparence_autre_melange["marquage_emoji"],
                        "marquage_libelle": apparence_autre_melange["marquage_libelle"],
                        "marquage_description": apparence_autre_melange["marquage_description"],   
                        "commentaire_global_images": images_commentaire_global,
                        "fiche_technique": {
                            "date_creation": date_creation,
                            "date_derniere_maj": date_creation,
        ...
        @st.dialog("Modifier une fiche de mélange", width="large", dismissible=True, on_dismiss=fermer_dialogue_actif)
        def ouvrir_modification_melange_complet(index, melange_data):
            ...
            with onglet_ft:
                ...
                images_commentaire_global = melange_data.get("commentaire_global_images", [])
                m_cause_modification = ""
                if activer_ft:
                    specs_dynamiques = render_dynamic_ft_inputs(
                        fiche_ancienne,
                    ...
                    m_cause_modification = st.text_input(
                        ...
                        key=f"pro_edit_mix_cause_{index}",                    
                    )
                    index_redacteur = (
        ...
        def pro_afficher_tableau_melanges(liste_tableau, prefixe_tableau):
            ...
            for melange_ligne in melanges_page:
                ...
                couleur_texte_ligne = style_ligne["couleur"]
                couleur_texte_secondaire = style_ligne["couleur_secondaire"]
                prefixe_ligne = style_ligne["prefixe"]
                tooltip_ligne = html_lib.escape(style_ligne.get("tooltip", ""))
                font_weight_ligne = "800" if style_ligne.get("gras") else "400"

                # Référence + emoji/marquage à gauche
                ref_txt = html_lib.escape(pro_texte(melange_data.get("ref")) or "-")
                ligne[0].markdown(
                    f'<span title="{tooltip_ligne}" style="color:{couleur_texte_ligne};font-weight:800;">{prefixe_ligne}{ref_txt}</span>',
                    unsafe_allow_html=True,
                )
                ...
                # Nom
                nom_txt = html_lib.escape(pro_texte(melange_data.get("nom")) or "-")
                ligne[1].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{nom_txt}</span>',
                    unsafe_allow_html=True,
                )
                ...
                if melange_data.get("image_rendu"):
                    ...
                    ligne[2].markdown(
                        f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">—</span>',
                        unsafe_allow_html=True,
                    )
                ...
                with ligne[3]:
                    for composant in composants[:8]:
                        ...
                        nom_composant = html_lib.escape(
                            pro_texte(composant.get("nom")) or "-"
                        )

                        dosage_composant = html_lib.escape(
                            pro_texte(composant.get("dosage")) or "-"
                        )

                        html_composant = (
                            f'<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;font-size:12px;color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">'
                            f'<span style="width:12px;height:12px;flex:0 0 12px;background:{couleur_composant};border:1px solid rgba(20,34,53,.30);border-radius:50%;"></span>'
                            f'<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:{couleur_texte_ligne};">{nom_composant}</span>'
                            f'<b style="margin-left:auto;white-space:nowrap;color:{couleur_texte_ligne};">{dosage_composant}</b>'
                            f'</div>'
                        )

                        st.markdown(html_composant, unsafe_allow_html=True)

                    if len(composants) > 8:
                        st.markdown(
                            f'<div style="font-size:12px;color:{couleur_texte_secondaire};font-weight:{font_weight_ligne};">+ {len(composants) - 8} autre(s)</div>',
                            unsafe_allow_html=True,
                        )
                    ...
                    if totaux:
                        ...
                        resume = html_lib.escape(resume)

                        st.markdown(
                            f'<div style="font-size:12px;color:{couleur_texte_secondaire};font-weight:{font_weight_ligne};margin-top:3px;">Total lisible : {resume}</div>',
                            unsafe_allow_html=True,
                        )

                # Emplacement
                emplacement_txt = html_lib.escape(
                    pro_texte(melange_data.get("emplacement")) or "-"
                )
                ligne[4].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{emplacement_txt}</span>',
                    unsafe_allow_html=True,
                )

                # Commentaire
                commentaire_txt = html_lib.escape(
                    pro_texte(melange_data.get("commentaire")) or "-"
                )
                ligne[5].markdown(
                    f'<span style="color:{couleur_texte_ligne};font-weight:{font_weight_ligne};">{commentaire_txt}</span>',
                    unsafe_allow_html=True,
                ...
                # ref_txt = html_lib.escape(pro_texte(melange_data.get("ref")) or "-")
                # ligne[0].markdown(
                #     f'<span style="color:{couleur_texte_ligne};font-weight:800;">{verification}{ref_txt}</span>',
                #     unsafe_allow_html=True,
                # )
                ...
                # nom_txt = html_lib.escape(pro_texte(melange_data.get("nom")) or "-")
                # ligne[1].markdown(
                #     f'<span style="color:{couleur_texte_ligne};">{nom_txt}</span>',
                #     unsafe_allow_html=True,
                # )

                # # Aperçu image ou tiret
                # if melange_data.get("image_rendu"):
                #     with ligne[2]:
                #         afficher_image_base64(
                #             melange_data["image_rendu"]["data"],
                ...
                # else:
                #     ligne[2].markdown(
                #         f'<span style="color:{couleur_texte_ligne};">—</span>',
                #         unsafe_allow_html=True,
                #     )
                ...
                #         html_composant = (
                #             f'<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;font-size:12px;color:{couleur_texte_ligne};">'
                #             f'<span style="width:12px;height:12px;flex:0 0 12px;background:{couleur_composant};border:1px solid rgba(20,34,53,.30);border-radius:50%;"></span>'
                #             f'<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:{couleur_texte_ligne};">{nom_composant}</span>'
                #             f'<b style="margin-left:auto;white-space:nowrap;color:{couleur_texte_ligne};">{dosage_composant}</b>'
                #             f'</div>'
                ...
                #     if len(composants) > 8:
                #         st.markdown(
                #             f'<div style="font-size:12px;color:{couleur_texte_secondaire};">+ {len(composants) - 8} autre(s)</div>',
                #             unsafe_allow_html=True,
                #         )
                ...
                #         st.markdown(
                #             f'<div style="font-size:12px;color:{couleur_texte_secondaire};margin-top:3px;">Total lisible : {resume}</div>',
                #             unsafe_allow_html=True,
                #         )
                ...
                # )
                # ligne[4].markdown(
                #     f'<span style="color:{couleur_texte_ligne};">{emplacement_txt}</span>',
                #     unsafe_allow_html=True,
                # )
                ...
                # )
                # ligne[5].markdown(
                #     f'<span style="color:{couleur_texte_ligne};">{commentaire_txt}</span>',
                #     unsafe_allow_html=True,
                # )
        ...
        with onglet_autres_melanges:
            st.markdown("##### Filtrer les Autres Mélanges par marquage visuel")

            afficher_legende_marquages("table_autres_melanges")

            col_filtre_couleur_autre, col_filtre_marque_autre = st.columns(2)

            with col_filtre_couleur_autre:
                options_filtre_couleurs = ["Toutes"] + [
                    option["id"] for option in OPTIONS_COULEURS_AUTRES_MELANGES
                ]

                filtre_couleur_autre = st.selectbox(
                    "Filtrer par couleur de texte",
                    options_filtre_couleurs,
                    key="filtre_autres_melanges_couleur",
                    format_func=lambda ident: (
                        "Toutes les couleurs"
                        if ident == "Toutes"
                        else get_option_couleur_autre_melange(ident)["label"]
            ...
            with col_filtre_marque_autre:
                options_filtre_marquages = ["Tous"] + [
                    option["code"] for option in OPTIONS_MARQUAGES_AUTRES_MELANGES
                ]

                filtre_marquage_autre = st.selectbox(
                    "Filtrer par marquage",
                    options_filtre_marquages,
                    key="filtre_autres_melanges_marquage",
                    format_func=lambda code: (
                        "Tous les marquages"
                        if code == "Tous"
                        else (
            ...
            for item_autre in autres_melanges:
                data_autre = item_autre["data"]

                couleur_item = get_couleur_depuis_melange(data_autre)
                marquage_item = get_marquage_depuis_melange(data_autre)

                if filtre_couleur_autre != "Toutes" and couleur_item["id"] != filtre_couleur_autre:
                    continue

                if filtre_marquage_autre != "Tous" and marquage_item["code"] != filtre_marquage_autre:
                    continue

                autres_melanges_vue.append(item_autre)

            pro_barre_resultats(
                len(autres_melanges_vue),
                len(autres_melanges),
                "autres mélanges",
            )            
            ...
            with st.expander("⚙️ Gérer / Supprimer les compartiments Atelier", expanded=False):
                ...
                # -----------------------------------------------------------
                st.markdown("### Renommer un compartiment Atelier")

                if liste_ateliers:

                    col_old_at, col_new_at, col_btn_rename_at = st.columns([2, 2, 1])

                    with col_old_at:
                        ancien_atelier = st.selectbox(
                            "Compartiment Atelier à renommer",
                            liste_ateliers,
                            key="select_rename_atelier_melange",
                        )

                    with col_new_at:
                        nouveau_nom_atelier_rename = st.text_input(
                            "Nouveau nom du compartiment Atelier",
                            key="input_rename_atelier_melange",
                            placeholder="Exemple : Atelier Coulée",
                        )

                    with col_btn_rename_at:
                        st.write("")
                        st.write("")

                        if st.button(
                            "✏️ Renommer",
                            key="btn_rename_atelier_melange",
                            use_container_width=True,
                        ):
                            nouveau_atelier_clean = nettoyer_nom_compartiment(
                                nouveau_nom_atelier_rename
                            )

                            if not ancien_atelier:
                                st.warning("Aucun compartiment Atelier sélectionné.")

                            elif not nouveau_atelier_clean:
                                st.warning("Veuillez saisir un nouveau nom.")

                            elif normaliser_nom_compartiment(ancien_atelier) == normaliser_nom_compartiment(nouveau_atelier_clean):
                                # Permet de corriger uniquement la casse / présentation
                                renommer_compartiment_melanges(
                                    liste_ateliers,
                                    liste_melanges,
                                    ancien_atelier,
                                    nouveau_atelier_clean,
                                )
                                sauvegarder_donnees()
                                st.rerun()
                                ...
                                st.error("Ce nom de compartiment Atelier existe déjà. Choisissez un autre nom.")
                                ...
                                sauvegarder_donnees()
                                afficher_animation_validation("Compartiment Atelier renommé")
                                st.rerun()

                else:
                    st.info("Aucun compartiment Atelier à renommer.")
                    
            # --- Sous-onglets par compartiment Atelier ---
            noms_onglets_ateliers = ["Tous les Autres Mélanges"] + [
                f"Atelier : {majuscule_initiale(at)}" for at in liste_ateliers
            ]
            onglets_ateliers = st.tabs(noms_onglets_ateliers)
        
            # Onglet Général (Tous)
            with onglets_ateliers[0]:
                pro_afficher_tableau_melanges(autres_melanges_vue, "autres_tous")
            ...
            for index_at, at_nom in enumerate(liste_ateliers):
                with onglets_ateliers[index_at + 1]:
                    melanges_atelier = [
                        item
                        for item in autres_melanges_vue
...
# =============================================================================
    ...
    st.title("🗑️ Corbeille — Historique des Suppressions (Ne videz pas la corbeille)")
    ...
    # Assure un identifiant stable pour chaque élément de corbeille.
    # Utile pour confirmer une suppression même si la liste est filtrée ou inversée.
    ids_corbeille_modifies = False

    for item_corbeille in corbeille:
        if not item_corbeille.get("id_corbeille"):
            item_corbeille["id_corbeille"] = str(uuid.uuid4())[:8]
            ids_corbeille_modifies = True

    if ids_corbeille_modifies:
        sauvegarder_donnees()
    # -------------------------------------------------------------------------
    # DIALOGUES DE MODIFICATION DÉDIÉS AUX ÉLÉMENTS EN CORBEILLE
    # -------------------------------------------------------------------------
    @st.dialog("Modifier une couleur (Corbeille)", width="large", dismissible=True, on_dismiss=fermer_dialogue_actif)
    ...
    @st.dialog("Modifier un élément (Corbeille)", width="large", dismissible=True, on_dismiss=fermer_dialogue_actif)
    ...
    @st.dialog("Modifier un mélange (Corbeille)", width="large", dismissible=True, on_dismiss=fermer_dialogue_actif)
    ...
    with col_vider:
        if corbeille:
            if st.button(
                ...
                st.session_state.confirm_clear_trash_word_reset = True
                st.rerun()

    # -------------------------------------------------------------------------
    # CONFIRMATION FORTE POUR VIDER TOUTE LA CORBEILLE
    # -------------------------------------------------------------------------
    if corbeille and st.session_state.get("confirm_clear_trash", False):

        if st.session_state.pop("confirm_clear_trash_word_reset", False):
            st.session_state["confirm_clear_trash_word"] = ""

        st.error(
            "⚠️ Vous êtes sur le point de vider DÉFINITIVEMENT toute la corbeille. "
            "Cette action est irréversible."
        )

        mot_vidage = st.text_input(
            "Pour confirmer le vidage complet, tapez exactement : VIDER",
            key="confirm_clear_trash_word",
            placeholder="Tapez VIDER ici",
        )

        confirmation_vidage_ok = mot_vidage.strip().upper() == "VIDER"

        c_oui, c_non = st.columns(2)

        if c_oui.button(
            "✅ Confirmer le vidage définitif",
            key="yes_clear_trash",
            disabled=not confirmation_vidage_ok,
            use_container_width=True,
        ):
    ...
    if not corbeille:
        ...
        for index_reel, item in enumerate(reversed(corbeille)):
            ...
            if b_edit.button("✎", key=f"trash_edit_{index_original}", help="Modifier l'élément archivé"):
                ouvrir_dialogue_actif(f"edit_trash_{item_type}", index_original)
                st.rerun()
            ...
            # 5. 🗑 SUPPRIMER DÉFINITIVEMENT AVEC CONFIRMATION

            id_corbeille = str(item.get("id_corbeille") or f"idx_{index_original}")
            id_corbeille_key = re.sub(r"[^a-zA-Z0-9_]+", "_", id_corbeille)

            if b_del.button(
                "🗑️",
                key=f"trash_del_{id_corbeille_key}",
                help="Demander confirmation avant suppression définitive",
            ):
                st.session_state.trash_delete_pending_id = id_corbeille
            ...
            if st.session_state.get("trash_delete_pending_id") == id_corbeille:
                ...
                if col_confirmer_del.button(
                    ...
                    index_a_supprimer = next(
                        (
                            i
                            for i, element_corbeille in enumerate(corbeille)
                            if str(element_corbeille.get("id_corbeille")) == id_corbeille
                        ),
                        None,
                    )
