...
def generer_pdf_fiche_technique(data_obj, type_ft="melange"):
    ...
    # AJOUT : Aperçu du produit dans le PDF
    if "image_rendu" in data_obj and data_obj["image_rendu"]:
        ...
        try:
            ...
            img_rl = RLImage(io.BytesIO(img_bytes), width=100, height=100)
            img_rl.hAlign = 'LEFT'
            story.append(img_rl)
        except Exception as e:
            story.append(Paragraph("Image indisponible.", body_style))
...
def ouvrir_visualisation_ft(melange):
    ...
    # 1. Barre d'infos dates (affiche maintenant l'heure grâce au changement de format)
